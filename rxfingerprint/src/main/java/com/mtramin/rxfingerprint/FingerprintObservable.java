...
abstract class FingerprintObservable<T> implements Observable.OnSubscribe<T> {
     ...
     */
    FingerprintObservable(Context context, FingerprintModule fingerprintModule) {
    ...
    public void call(Subscriber subscriber) {
        ...
        AuthenticationCallbackWrapper callback = createAuthenticationCallback(subscriber);
        cancellationSignal = fingerprintModule.createCancellationSignal();
        CryptoObjectWrapper cryptoObject = initCryptoObject(subscriber);
        fingerprintModule.authenticate(context, cryptoObject, cancellationSignal, 0, callback, null);
    ...
    private AuthenticationCallbackWrapper createAuthenticationCallback(final Subscriber<T> subscriber) {
        return new AuthenticationCallbackWrapper() {
            @Override
            ...
            @Override
            public void onAuthenticationSucceeded(CryptoObjectWrapper result) {
                if (!subscriber.isUnsubscribed()) {
     ...
     */
    @Nullable
    protected abstract CryptoObjectWrapper initCryptoObject(Subscriber<T> subscriber);
     ...
     *
     * @param subscriber   current subscriber
     * @param cryptoObject result of the successful fingerprint authentication
     */
    protected abstract void onAuthenticationSucceeded(Subscriber<T> subscriber, CryptoObjectWrapper cryptoObject);
